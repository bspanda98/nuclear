...
var (
	...
	//
	energiAuthorsFileHeader = "# This is the official list of Nuclear Core authors for copyright purposes.\n\n"
	energiAuthorsRE         = regexp.MustCompile(`@(energi.team|futoin)`)

	// The commit after which the Nuclear source starts
	//energiForkPoint = "4bcc0a37ab70cb79b16893556cffdaad6974e7d8"

	// Date of Nuclear fork refactoring start
...
// Copyright {{.YearNuclear}} The Nuclear Core Authors
// Copyright {{.Year}} The go-ethereum Authors
// This file is part of {{.WholeNuclear false}}.
//
// {{.WholeNuclear true}} is free software: you can redistribute it and/or modify
// it under the terms of the GNU {{.License}} as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// {{.WholeNuclear true}} is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU {{.License}} for more details.
//
// You should have received a copy of the GNU {{.License}}
// along with {{.WholeNuclear false}}. If not, see <http://www.gnu.org/licenses/>.

`[1:]))

var onlyNuclearLicenseT = template.Must(template.New("").Parse(`
// Copyright {{.YearNuclear}} The Nuclear Core Authors
// This file is part of {{.WholeNuclear false}}.
//
// {{.WholeNuclear true}} is free software: you can redistribute it and/or modify
...
func (i info) WholeNuclear(startOfSentence bool) string {
	if i.gpl() {
		return "Nuclear Core"
	}
	if startOfSentence {
		return "The Nuclear Core library"
	}
	return "the Nuclear Core library"
}
...
func fileInfo(file string) (*info, error) {
	info := &info{file: file, Year: int64(time.Now().Year()), IsNuclear: false}
	...
	err := doLines(cmd, func(line string) {
		...
		if line[:len(energiForkDate)] >= energiForkDate {
			info.IsNuclear = true
			info.OnlyNuclear = true
		}
		if line[:len(energiForkDate)] < energiForkDate {
			info.OnlyNuclear = false
		}
	...
	if info.Year >= energiStartYear {
		info.YearNuclear = info.Year
		// TODO: revise on cherry-picks
		info.Year = energiStartYear
	} else {
		info.YearNuclear = energiStartYear
	}
	return info, err
}
...
func writeLicense(info *info) {
	...
	if info.IsNuclear {
		if info.OnlyNuclear {
			onlyNuclearLicenseT.Execute(buf, info)
		} else {
