Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for nuclear's account management.
This allows DApps not to depend on nuclear's account management. When a DApp wants to sign data it can send the data to
the signer, the signer will then provide the user with context and asks the user for permission to sign the data. If
the users grants the signing request the signer will send the signature back to the DApp.
  
This setup allows a DApp to connect to a remote Nuclear node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Nuclear node is not available, not
synchronised with the chain or a particular Nuclear node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --networkid value       Network identifier (integer, 39797=NuclearMain, 49797=NuclearTest) (default: 39797)
...
The general flow for signing a transaction using e.g. nuclear is as follows:
![image](sign_flow.png)

In this case, `nuclear` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Nuclear Core should be started in `nuclear --external_signer localhost:8550`.
    - Currently, the Nuclear Core APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Nuclear Core api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus nuclear/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Nuclear Core. The messages are
  ...
  "params": [
    {
      "address": "0x123409812340981234098123409812deadbeef42",
      "raw_data": "0x01020304",
      "message": "\u0017Nuclear Signed Message:\n4\u0001\u0002\u0003\u0004",
