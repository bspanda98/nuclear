...
func TestConsoleWelcome(t *testing.T) {
	...
	geth.SetTemplateFunc("niltime", func() string {
		return time.Unix(int64(core.DefaultNuclearMainnetGenesisBlock().Timestamp), 0).Format(time.RFC1123)
	})
	geth.SetTemplateFunc("apis", func() string { return ipcAPIs })

	// Verify the actual welcome message to the required template
	geth.Expect(`
Welcome to the Nuclear Core JavaScript console!

instance: nuclear/v{{gethver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\nuclear` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "nuclear.ipc")
	}
