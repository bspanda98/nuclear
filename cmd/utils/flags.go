...
import (
	...
	"nuclear/core/nuclear/accounts"
	"nuclear/core/nuclear/accounts/keystore"
	"nuclear/core/nuclear/common"
	"nuclear/core/nuclear/common/fdlimit"
	"nuclear/core/nuclear/consensus"
	"nuclear/core/nuclear/consensus/clique"
	...
	"nuclear/core/nuclear/dashboard"
	"nuclear/core/nuclear/eth"
	"nuclear/core/nuclear/eth/downloader"
	"nuclear/core/nuclear/eth/gasprice"
	"nuclear/core/nuclear/ethdb"
	"nuclear/core/nuclear/ethstats"
	"nuclear/core/nuclear/les"
	"nuclear/core/nuclear/log"
	"nuclear/core/nuclear/metrics"
	"nuclear/core/nuclear/metrics/influxdb"
	"nuclear/core/nuclear/node"
	"nuclear/core/nuclear/p2p"
	"nuclear/core/nuclear/p2p/discv5"
	"nuclear/core/nuclear/p2p/enode"
	"nuclear/core/nuclear/p2p/nat"
	"nuclear/core/nuclear/p2p/netutil"
	"nuclear/core/nuclear/params"
	whisper "nuclear/core/nuclear/whisper/whisperv6"
	cli "gopkg.in/urfave/cli.v1"

	energi "nuclear/core/nuclear/energi/consensus"
	energi_svc "nuclear/core/nuclear/energi/service"
)
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 39797=NuclearMain, 49797=NuclearTest)",
		Value: eth.DefaultConfig.NetworkId,
	}
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "NuclearTest network: pre-configured proof-of-work test network",
	}
	...
	MasternodeFlag = cli.BoolFlag{
		Name:  "masternode",
		Usage: "Enable Nuclear Masternode service",
	}
	...
	NuclearInitDevFlag = cli.StringFlag{
		Name:  "init",
		Usage: "Nuclear network: use pre-configured custom genesis block",
		Value: "",
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `nuclear account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	checkExclusive(ctx, DeveloperFlag, TestnetFlag, NuclearInitDevFlag)
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		cfg.Genesis = core.DefaultNuclearTestnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
	...
	case ctx.GlobalString(NuclearInitDevFlag.Name) != "":
		// Check for the pre-defined custom genesis block configuration.
		var err error
		cfg.Genesis, err = core.DeveloperNuclearGenesisBlock(ctx.GlobalString(NuclearInitDevFlag.Name))
		if err != nil {
			Fatalf("Valid custom genesis block configuration was not found: %v", err)
		}
	default:
		cfg.Genesis = core.DefaultNuclearMainnetGenesisBlock()
	}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Nuclear service: %v", err)
	}
}
...
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Nuclear Stats service: %v", err)
	}
}

// Configure Nuclear Dynamic Checkpoint service
func RegisterDynamicCheckpointService(stack *node.Node) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Nuclear Checkpoint service: %v", err)
	}
}

// RegisterMasternodeService configures Nuclear Masternode service. It also accepts
...
func RegisterMasternodeService(stack *node.Node, owner common.Address) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Nuclear Masternode service: %v", err)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	if config.Nuclear != nil {
		engine = energi.New(config.Nuclear, chainDb)
