...
func TestNuclearGenesisBlock(t *testing.T) {
	block := DefaultNuclearMainnetGenesisBlock().ToBlock(nil)
	if block.Hash() != params.MainnetGenesisHash {
		t.Errorf("wrong energi mainnet genesis hash, got %v, want %v", block.Hash().String(), params.MainnetGenesisHash.String())
	}
	block = DefaultNuclearTestnetGenesisBlock().ToBlock(nil)
...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				DefaultNuclearMainnetGenesisBlock().MustCommit(db)
				return SetupGenesisBlock(db, nil)
			},
			wantHash:   params.MainnetGenesisHash,
			wantConfig: DefaultNuclearMainnetGenesisBlock().Config,
		},
		{
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				customg.MustCommit(db)
				return SetupGenesisBlock(db, DefaultNuclearTestnetGenesisBlock())
			},
			wantErr:    &GenesisMismatchError{Stored: customghash, New: params.TestnetGenesisHash},
			wantHash:   params.TestnetGenesisHash,
			wantConfig: params.NuclearTestnetChainConfig,
		},
		{
