...
func (cm *checkpointManager) setup(chain CheckpointChain) {
	...
	if checkpoints, ok := energi_params.NuclearCheckpoints[genesis_hash]; ok {
...
func (cm *checkpointManager) addCheckpoint(
	...
	if !local {
		...
		genesis_hash := chain.GetHeaderByNumber(0).Hash();
		for maxHardcodedCheckpoint = range energi_params.NuclearCheckpoints[genesis_hash] {
			break
		}
		for n := range energi_params.NuclearCheckpoints[genesis_hash] {
		...
		if nrgconf := chain.Config().Nuclear; nrgconf == nil || signer != nrgconf.CPPSigner {
...
func (cm *checkpointManager) hashToSign(cp *Checkpoint) []byte {
	data := []byte("||Nuclear Blockchain Checkpoint||")
