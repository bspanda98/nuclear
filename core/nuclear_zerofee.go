 ...
 *
 * Check if Nuclear consensus allows the transaction to be processed as
 * zero-fee.
 */
...
func IsGen2Migration(tx *types.Transaction) bool {
	...
	return (to != nil) &&
		(*to == energi_params.Nuclear_MigrationContract) &&
...
func IsMasternodeCall(tx *types.Transaction) bool {
	...
	if (to == nil) || (*to != energi_params.Nuclear_MasternodeRegistry) {
		return false
	}
...
func IsCheckpointCall(tx *types.Transaction) bool {
	...
	if (to == nil) || (*to != energi_params.Nuclear_CheckpointRegistry) {
		return false
	}
...
func IsBlacklisted(db vm.StateDB, addr common.Address) bool {
	return db.GetState(energi_params.Nuclear_Blacklist, addr.Hash()) != common.Hash{}
}

func IsWhitelisted(db vm.StateDB, addr common.Address) bool {
	return db.GetState(energi_params.Nuclear_Whitelist, addr.Hash()) != common.Hash{}
}
...
func (z *zeroFeeProtector) checkMasternode(
	...
	mn_indicator := pool.currentState.GetState(
		energi_params.Nuclear_MasternodeList, sender.Hash())
