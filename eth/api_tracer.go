...
import (
	...
	"nuclear/core/nuclear/eth/tracers"
...
func (api *PrivateDebugAPI) handleConsensusTx(
	...
	if tx.IsConsensus() {
		if addr := tx.ConsensusSender(); addr == energi_params.Nuclear_SystemFaucet {
			statedb.SetBalance(energi_params.Nuclear_SystemFaucet, tx.Value())
		} else if (statedb.GetState(energi_params.Nuclear_Blacklist, addr.Hash()) != common.Hash{}) {
