...
import (
	...
	"nuclear/core/nuclear/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Nuclear peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Nuclear protocol")
	...
	log.Info("Nuclear protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Nuclear peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Nuclear handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Nuclear peer registration failed", "err", err)
		return err
	}
	...
	if err := p.RequestCheckpoints(); err != nil {
		p.Log().Error("Nuclear checkpoint request failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Nuclear message handling failed", "err", err)
			return err
		}
	}
}
