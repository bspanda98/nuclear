...
import (
	...
	"nuclear/core/nuclear/common/mclock"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Nuclear protocol")
	...
	log.Info("Light Nuclear protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Nuclear peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Nuclear handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Nuclear peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Nuclear message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Nuclear message arrived", "code", msg.Code, "bytes", msg.Size)
