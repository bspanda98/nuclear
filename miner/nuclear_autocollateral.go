// Copyright 2020 The Nuclear Core Authors
...
func (w *worker) tryAutocollateral() {
	...
	if _, ok := w.engine.(*energi.Nuclear); !ok {
		// Nuclear consensus engine not running.
	...
	// Get rewards
	mnReward, err := w.getBlockReward(energi_params.Nuclear_MasternodeRegistry, block.Number())
	if err != nil {
		log.Error(err.Error())
		return
	}
...
func (w *worker) tokenRegistry(dst common.Address) (*energi_abi.IMasternodeTokenSession, error) {
	contract, err := energi_abi.NewIMasternodeToken(
		energi_params.Nuclear_MasternodeToken, w.apiBackend.(bind.ContractBackend))
	if err != nil {
		return nil, err
	}
...
func (w *worker) collateralLimits() (minCollateral, maxCollateral *big.Int, err error) {
	registry, err := energi_abi.NewIMasternodeRegistryV2(
		energi_params.Nuclear_MasternodeRegistry, w.apiBackend.(bind.ContractBackend))
	if err != nil {
		return nil, nil, err
	}
