...
import (
	...
	"nuclear/core/nuclear/event"
	"nuclear/core/nuclear/params"

	energi_testutils "nuclear/core/nuclear/energi/common/testutils"
...
func init() {
	...
	energiChainConfig = params.NuclearTestnetChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, n int) *testWorkerBackend {
	...
	case *energi.Nuclear:
		...
		gspec.Alloc[energi_params.Nuclear_MigrationContract] = core.GenesisAccount{Balance: testBankFunds}
		energiEngine := engine.(*energi.Nuclear)
		// energiEngine.testing = true
		energiEngine.SetMinerCB(
			func() []common.Address {
				return []common.Address{energi_params.Nuclear_MigrationContract}
			},
		...
		chainConfig.Nuclear = &params.NuclearConfig{
		...
		gspec.Coinbase = energi_params.Nuclear_Treasury
		gspec.Xfers = core.DeployNuclearGovernance(chainConfig)
...
func TestPendingStateAndBlockNuclear(t *testing.T) {
	testPendingStateAndBlock(t, energiChainConfig, energi.New(energiChainConfig.Nuclear, ethdb.NewMemDatabase()))
}
...
// }
func TestEmptyWorkNuclear(t *testing.T) {
	testEmptyWork(t, energiChainConfig, energi.New(energiChainConfig.Nuclear, ethdb.NewMemDatabase()))
}
...
func TestRegenerateMiningBlockNuclear(t *testing.T) {
	testRegenerateMiningBlock(t, energiChainConfig, energi.New(energiChainConfig.Nuclear, ethdb.NewMemDatabase()))
}
...
func TestAdjustIntervalNuclear(t *testing.T) {
	testAdjustInterval(t, energiChainConfig, energi.New(energiChainConfig.Nuclear, ethdb.NewMemDatabase()))
}
