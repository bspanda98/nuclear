...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "nuclear-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/nucleartest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "nuclear/core/nuclear/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "nuclear.aar"), "nuclear.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "nucleartest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.ethereum.nucleartest"
