...
import (
	...
	energi_common "nuclear/core/nuclear/energi/common"
...
func (g *GovernanceAPI) upgradeInfo(num *big.Int) (interface{}, error) {
	...
	ret.Treasury, err = g.upgradeProposalInfo(num, energi_params.Nuclear_Treasury)
	if err != nil {
		log.Error("Treasury info fetch failed", "err", err)
	}

	ret.MasternodeRegistry, err = g.upgradeProposalInfo(num, energi_params.Nuclear_MasternodeRegistry)
	if err != nil {
		log.Error("MasternodeRegistry info fetch failed", "err", err)
	}

	ret.StakerReward, err = g.upgradeProposalInfo(num, energi_params.Nuclear_StakerReward)
	if err != nil {
		log.Error("StakerReward info fetch failed", "err", err)
	}

	ret.BackboneReward, err = g.upgradeProposalInfo(num, energi_params.Nuclear_BackboneReward)
	if err != nil {
		log.Error("BackboneReward info fetch failed", "err", err)
	}

	ret.SporkRegistry, err = g.upgradeProposalInfo(num, energi_params.Nuclear_SporkRegistry)
	if err != nil {
		log.Error("SporkRegistry info fetch failed", "err", err)
	}

	ret.CheckpointRegistry, err = g.upgradeProposalInfo(num, energi_params.Nuclear_CheckpointRegistry)
	if err != nil {
		log.Error("CheckpointRegistry info fetch failed", "err", err)
	}

	ret.BlacklistRegistry, err = g.upgradeProposalInfo(num, energi_params.Nuclear_BlacklistRegistry)
	if err != nil {
		log.Error("BlacklistRegistry info fetch failed", "err", err)
	}

	ret.MasternodeToken, err = g.upgradeProposalInfo(num, energi_params.Nuclear_MasternodeToken)
	if err != nil {
...
func (g *GovernanceAPI) budgetInfo(num *big.Int) (interface{}, error) {
	return treasuryInfo(energi_params.Nuclear_Treasury, g.backend)
}
...
func (g *GovernanceAPI) BudgetPropose(
	...
	return treasuryPropose(
		g.backend, energi_params.Nuclear_Treasury,
		amount, ref_uuid,
		period, fee,
		payer, password,
	)
