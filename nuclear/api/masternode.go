...
func (m *MasternodeAPI) token(
	...
	contract, err := energi_abi.NewIMasternodeToken(
		energi_params.Nuclear_MasternodeToken, m.backend.(bind.ContractBackend))
	if err != nil {
		return nil, err
	}
...
func (m *MasternodeAPI) CollateralBalance(
	...
	token, err := energi_abi.NewIMasternodeTokenCaller(
		energi_params.Nuclear_MasternodeToken, m.backend.(bind.ContractCaller))
	if err != nil {
...
func (m *MasternodeAPI) listMasternodes(num *big.Int) (interface{}, error) {
	registry, err := energi_abi.NewIMasternodeRegistryV2Caller(
		energi_params.Nuclear_MasternodeRegistry, m.backend.(bind.ContractCaller))
	if err != nil {
		log.Error("Failed", "err", err)
		return nil, err
	}
...
func masternodeRegistry(
	...
	contract, err := energi_abi.NewIMasternodeRegistryV2(
		energi_params.Nuclear_MasternodeRegistry, backend.(bind.ContractBackend))
	if err != nil {
		return nil, err
	}
