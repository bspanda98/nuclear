...
import (
	...
	energi_consensus "nuclear/core/nuclear/energi/consensus"
...
func (m *MigrationAPI) listGen2CoinsUncached(num *big.Int) (interface{}, error) {
	...
	currBalance := state.GetBalance(energi_params.Nuclear_MigrationContract)
	...
	mgrt_contract, err := energi_abi.NewGen2MigrationCaller(
		energi_params.Nuclear_MigrationContract, m.backend.(bind.ContractCaller))
	if err != nil {
...
// SearchGen3DestinationByGen2Address returns the nuclear destination address(s) searched
...
func (m *MigrationAPI) SearchGen3DestinationByGen2Address(
	...
	mgrtContract, err := energi_abi.NewGen2Migration(
		energi_params.Nuclear_MigrationContract, m.backend.(bind.ContractBackend))
	if err != nil {
		log.Error("Failed to create contract face", "err", err)
		return nil, err
	}
