...
func (e *Nuclear) processBlacklists(
	chain ChainReader,
	header *types.Header,
	statedb *state.StateDB,
) (err error) {
	blregistry := energi_params.Nuclear_BlacklistRegistry
	...
	state_obj := statedb.GetOrNewStateObject(energi_params.Nuclear_Blacklist)
	db := statedb.Database()
	...
	//
	wl_state_obj := statedb.GetOrNewStateObject(energi_params.Nuclear_Whitelist)
...
var (
	...
	consensusStandalone = []common.Address{
		...
		energi_params.Nuclear_SystemFaucet,
	}
)

func (e *Nuclear) createWhitelist(
	statedb *state.StateDB,
...
func (e *Nuclear) processDrainable(
	chain ChainReader,
	header *types.Header,
	statedb *state.StateDB,
	...
	for _, addr := range *address_list {
		...
		statedb.SetState(energi_params.Nuclear_Blacklist, addr.Hash(), common.Hash{})
		evm = e.createEVM(msg, chain, header, statedb)
		gp = core.GasPool(msg.Gas())
		_, gas1, failed, err := core.ApplyMessage(evm, msg, &gp)
		statedb.SetState(energi_params.Nuclear_Blacklist, addr.Hash(), blacklistValue)
