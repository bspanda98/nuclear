...
func TestBlacklist(t *testing.T) {
	...
	engine := New(&params.NuclearConfig{}, testdb)

	engine.testing = true

	chainConfig := *params.NuclearTestnetChainConfig
	...
	var (
		gspec = &core.Genesis{
			...
			Xfers:      core.DeployNuclearGovernance(&chainConfig),
		}
	...
	callData, err = blacklist_abi.Pack("propose", energi_params.Nuclear_TreasuryV1)
	assert.Empty(t, err)
	fee = new(big.Int).Mul(big.NewInt(1000), big.NewInt(1e18))
	blstate.SetBalance(owner_addr, fee)
	msg = types.NewMessage(
	...
	evm = engine.createEVM(msg, chain, header, blstate)
	callData, err = blacklist_abi.Pack("proposeDrain", energi_params.Nuclear_TreasuryV1)
	assert.Empty(t, err)
	fee = new(big.Int).Mul(big.NewInt(100), big.NewInt(1e18))
	blstate.SetBalance(owner_addr, fee)
	msg = types.NewMessage(
	...
	blstate.AddBalance(energi_params.Nuclear_TreasuryV1, amt)
	...
	assert.False(t, core.CanTransfer(blstate, blacklist_addr1, common.Big1))
	assert.True(t, core.CanTransfer(blstate, blacklist_addr2, common.Big1))
	assert.True(t, core.CanTransfer(blstate, energi_params.Nuclear_TreasuryV1, common.Big1))
	txs, receipts, err = engine.processDrainable(chain, header, blstate, nil, nil)
	assert.Empty(t, err)
	assert.Empty(t, txs)
	assert.Empty(t, receipts)
	...
	assert.NotEqual(t, blstate.GetBalance(energi_params.Nuclear_TreasuryV1).String(), common.Big0.String())
	header.Root, err = blstate.Commit(true)
	assert.Empty(t, err)
	err = blstate.Database().TrieDB().Commit(header.Root, true)
	assert.Empty(t, err)
