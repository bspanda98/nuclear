...
func generateAddresses(len int) ([]common.Address, map[common.Address]*ecdsa.PrivateKey, core.GenesisAlloc, common.Address) {
	...
	alloc[energi_params.Nuclear_MigrationContract] = core.GenesisAccount{
		Balance: minStake,
	}

	migrationSigner := addresses[len-1]
	signers[energi_params.Nuclear_MigrationContract] = signers[migrationSigner]
...
func TestPoSChain(t *testing.T) {
	...
	engine := New(&params.NuclearConfig{MigrationSigner: migrationSigner}, testdb)
