...
func (m *MasternodeService) Start(server *p2p.Server) error {
	...
	contract, err := energi_abi.NewIMasternodeRegistryV2(
		energi_params.Nuclear_MasternodeRegistry, m.eth.APIBackend)
	if err != nil {
		return err
	}
	...
	cpContract, err := energi_abi.NewICheckpointRegistry(
		energi_params.Nuclear_CheckpointRegistry, m.eth.APIBackend)
	if err != nil {
		return err
	}
	...
	log.Info("Started Nuclear Masternode", "addr", address)
	return nil
}

func (m *MasternodeService) Stop() error {
	log.Info("Shutting down Nuclear Masternode", "addr", m.address)
	m.validator.cancel()
	return nil
}
