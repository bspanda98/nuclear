...
import (
	...
	"nuclear/core/nuclear/accounts/abi/bind"
...
func TestMasternodeService(t *testing.T) {
	...
	delegatedPOS := []common.Address{
		...
		params.NuclearTestnetChainConfig.Nuclear.CPPSigner,
	}
	...
	for _, addr := range delegatedPOS {
		...
		switch addr {
		case energi_params.Nuclear_MigrationContract:
			mgSigner = accAddr

		case params.NuclearTestnetChainConfig.Nuclear.CPPSigner:
			cpSigner = accAddr
		}
	}
	...
	err := migrations.PrepareTestGen2Migration(params.NuclearTestnetChainConfig.ChainID.Uint64())
...
func serviceConfig(presale core.GenesisAlloc) *eth.Config {
	...
	ethConfig.Genesis = core.DefaultNuclearTestnetGenesisBlock()
	ethConfig.Genesis.GasLimit = gasLimit
	ethConfig.Genesis.Config.ChainID = big.NewInt(1)
	ethConfig.Genesis.Config.Nuclear.CPPSigner = cpSigner
	ethConfig.Genesis.Config.Nuclear.MigrationSigner = mgSigner
	ethConfig.Genesis.Alloc = presale
	ethConfig.Genesis.Xfers = core.DeployNuclearGovernance(ethConfig.Genesis.Config)
	ethConfig.Genesis.Difficulty = big.NewInt(1)
	ethConfig.Genesis.Coinbase = energi_params.Nuclear_Treasury
...
func cpPropose() (*core.CheckpointInfo, error) {
	...
	cpRegistry, err := energi_abi.NewCheckpointRegistryV2(
		energi_params.Nuclear_CheckpointRegistry, ethServ.APIBackend,
	)
	if err != nil {
		return nil, err
	}
...
func depositCollateral(
	...
	mnRegistry, err := energi_abi.NewIMasternodeTokenTransactor(
		energi_params.Nuclear_MasternodeToken, ethService.APIBackend)
	if err != nil {
		return err
	}
...
func announceMN(
	...
	registry, err := energi_abi.NewIMasternodeRegistryV2Transactor(
		energi_params.Nuclear_MasternodeRegistry, ethService.APIBackend)
	if err != nil {
		return err
	}
...
func isSignedCheckpointTx(tx *types.Transaction) bool {
	if *tx.To() == energi_params.Nuclear_CheckpointRegistry {
