...
To run Swarm you need an Nuclear account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Nuclear address representing that account. For example:
...
### Nuclear Name Service resolution

The Nuclear Name Service is the Nuclear equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `nuclear` instance, which is synced with the Nuclear mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.ethereum/nuclear.ipc'
