...
#
# The cluster contains a bootnode, a nuclear node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the nuclear and
# swarm nodes
start_bootnode() {
...
# start_nuclear_node starts a nuclear node with --datadir pointing at <base-dir>/nuclear
# and a single, unlocked account with password "nuclear"
start_nuclear_node() {
  local dir="${base_dir}/nuclear"
  mkdir -p "${dir}"

  local password="nuclear"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating nuclear account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get nuclear account address"
  fi
  ...
  start_node "nuclear" "${GETH_IP}" "$(which nuclear)" ${args[@]}
}
...
create_account() {
  ...
  nuclear --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
